/* Functions for the ezimage plugin popup */
...
function init() {
	...
	// Src browser
	html = getBrowserHTML('srcbrowser','src','image','ezimage');
	document.getElementById("srcbrowsercontainer").innerHTML = html;

	// Over browser
	html = getBrowserHTML('oversrcbrowser','onmouseoversrc','image','ezimage');
	document.getElementById("onmouseoversrccontainer").innerHTML = html;

	// Out browser
	html = getBrowserHTML('outsrcbrowser','onmouseoutsrc','image','ezimage');
	document.getElementById("onmouseoutsrccontainer").innerHTML = html;

	// Longdesc browser
	html = getBrowserHTML('longdescbrowser','longdesc','file','ezimage');
	...
	if (action == "update") {
		...
		addClassesToList('classlist', 'ezimage_styles');
	...
	if (tinyMCE.getParam("ezimage_constrain_proportions", true))
...
function insertAction() {
	...
	if (tinyMCE.getParam("accessibility_warnings")) {
		if (formObj.alt.value == "" && !confirm(tinyMCE.getLang('lang_ezimage_missing_alt', '', true)))
			return;
	}
...
function showPreviewImage(src, start) {
	...
	if (!start && tinyMCE.getParam("ezimage_update_dimensions_onchange", true))
		resetImageData();
